...
const uint8_t mdc1200_pre_amble[] = {0x00, 0xFF};
...
void xor_modulation(void *data, const unsigned int size) {    // exclusive-or succesive bits - the entire packet
    unsigned int i;
    uint8_t *data8 = (uint8_t *) data;
    uint8_t prev_bit = 0;  // Start with 0 to match the first preamble byte (0x00)
    for (i = 0; i < size; i++) {
        int bit_num;
        uint8_t in = data8[i];
        uint8_t out = 0;
...
// Append a short post-tone to the MDC1200 packet so the radio sends a
// brief 1800 Hz tone (Hytera-style) at the end of the packet. This is
// transmitted as extra FSK bits (not a UI sound).
//#define MDC1200_POST_TONE_MS   20
//#define MDC1200_BITRATE        1200
//#define MDC1200_POST_TONE_BITS ((MDC1200_POST_TONE_MS * MDC1200_BITRATE) / 1000)
/* Force 2 bytes (16 bits) of post-tone as requested */
//define MDC1200_POST_TONE_BYTES 2
///#define MDC1200_POST_TONE_BYTES ((MDC1200_POST_TONE_BITS + 7) / 8)
...
unsigned int MDC1200_encode_single_packet(void *data, const uint8_t op, const uint8_t arg, const uint16_t unit_id) {
    ...
    // size of the main MDC packet (without the post-tone)
    size = (unsigned int) (p - (uint8_t *) data);

    // apply XOR modulation to the main packet only
    xor_modulation(data, size);

    // append final post-tone bytes in POST-XOR (final) form so they transmit as 1800Hz ('space') like Hytera
//    memset((uint8_t *)data + size, 0x00, MDC1200_POST_TONE_BYTES);
//    size += MDC1200_POST_TONE_BYTES;
...
bool mdc1200_contact_find(uint16_t mdc_id, char *contact) {
    ...
    for (uint8_t i = 0; i < contact_num; i++) {
        ...
        if (mdc_id == (uint16_t) (read_once[1] | (read_once[0] << 8))) {
            // Validate contact name: check each character is printable ASCII
            // Stop validation at null terminator or max 14 characters
            int valid_len = 0;
            for (int j = 0; j < 14; ++j) {
                uint8_t c = read_once[2 + j];
                if (c == '\0') {
                    // Found null terminator - valid end of string
                    break;
                }
                if (c < ' ' || c > '~') {
                    // Found invalid character - string is corrupted
                    return false;
                }
                valid_len++;
            }

            // Copy the validated contact name
