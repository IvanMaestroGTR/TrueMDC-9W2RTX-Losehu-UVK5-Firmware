...
void BK4819_PlayRogerNormal(void) {
    ...
    // First tone: F7 (2637 Hz) - 250ms
    BK4819_WriteRegister(BK4819_REG_71, scale_freq(tone1_Hz));
    BK4819_ExitTxMute();
    SYSTEM_DelayMs(50);
    BK4819_EnterTxMute();

    // Second tone: DFlat7 (2489 Hz) - 250ms - no gap
...
// Helper function to send raw FSK packet data (for preamble tones)
static void BK4819_send_FSK_packet(const uint8_t *packet, unsigned int size)
{
    uint16_t fsk_reg59;
    ...
    {
        unsigned int timeout = 300 / 4;  // allow up to 300ms
        while (timeout-- > 0) {
            SYSTEM_DelayMs(4);
